...
public class BienLai extends KhachHang {
	...
	public void show() {
		System.out.printf("%-5s %-20s %-20s %10s %10s %10s %10s", "STT", "Tên", "Địa chỉ","Mã Công tơ","Số điện cũ","Số điện mới","Số tiền");
		System.out.println();
		for (int i = 0; i < listKH.size(); i++) {
			System.out.printf("\n %-4s %-20s %-20s %10s %10s %10s %10s", (i + 1), listKH.get(i).getHoTen(), listKH.get(i).getSoNha(),listKH.get(i).getMaSoCongTo(),listKH.get(i).getOldNumber(),listKH.get(i).getNewNumber(),listKH.get(i).getTienDien());
		}
		System.out.println();
		System.out.println();
	}
